...
class EnvironmentCleanup:
    ...
    def cleanup_test_config_folder():
        ...
        The delay -> retry is used as MusicLedStudio can take a bit of time to shut down and release the
    ...
    def mls_is_alive():
        ...
        Returns:
            bool: True if MusicLedStudio is running, False otherwise.
        """
        try:
            response = requests.get(
                f"http://{SERVER_PATH}/api/info", timeout=1
            ...
            if response.status_code == 200:
                # MusicLedStudio has returned a response, so it is running, but likely still hydrating the schema
