...
const BladeScene = ({ onClick }: { onClick: () => void }) => {
  ...
  const addBladeScene = () => {
    ...
    large.map((v) => {
      ...
      return activatePreset(v, 'mls_presets', 'melt', 'purple-red')
    })
    medium.map((v, _i) => {
    ...
    if (Object.keys(smalls).length === 0 && Object.keys(medium).length > 1) {
      ...
      setEffect(v, 'blade_power_plus', {}, true)
      activatePreset(v, 'mls_presets', 'blade_power_plus', 'orange-hi-hat')
    }
    // Use medium as large
    if (Object.keys(large).length === 0 && Object.keys(medium).length > 2) {
      ...
      setEffect(v, 'melt', {}, true)
      activatePreset(v, 'mls_presets', 'melt', 'purple-red')
    }
    ...
    if (Object.keys(medium).length === 0 && Object.keys(large).length > 2) {
      ...
      setEffect(v, 'blade_power_plus', {}, true)
      activatePreset(v, 'mls_presets', 'blade_power_plus', 'purplered-bass')
    }
