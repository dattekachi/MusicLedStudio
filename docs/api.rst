...
Information about MusicLedStudio

.. rubric:: GET

Returns basic information about the MusicLedStudio instance as JSON
...
Endpoint for managing MusicLedStudio Configuration

.. rubric:: GET

Returns the current configuration for MusicLedStudio as JSON
A GET with no request body will return MusicLedStudio's full configuration.
...
example: Get MusicLedStudio audio configuration

.. code-block:: json

    "audio"

example: Get MusicLedStudio audio, devices, and scenes configuration
...
Updates MusicLedStudio configuration with any permitted keys.
MusicLedStudio will handle the update and restart if necessary.
...
Set full MusicLedStudio config. You must provide a full, valid config.
MusicLedStudio will restart to apply the config.
To simply update a part of the config, use PUT

.. rubric:: DELETE

Resets MusicLedStudio's config to default values and restarts.
...
MusicLedStudio Logging

.. rubric:: GET

Opens a websocket connection through which realtime MusicLedStudio logging info will be sent.

/api/schema/
=========================

MusicLedStudio Schema Api

.. rubric:: GET /api/schema/

Get JSON schemas specifically defining the kind of data MusicLedStudio's API expects.
A GET with no request body will return all of MusicLedStudio's schemas
MusicLedStudio uses schemas to validate the following:
...
example: Get MusicLedStudio audio schema

.. code-block:: json

    "audio"

example: Get MusicLedStudio devices and effects schema
...
Query a specific MusicLedStudio schema with the matching *schema_type* as JSON

.. rubric:: GET /api/schema/<schema_type>

Returns the MusicLedStudio schema with the matching *schema_type* as JSON

- *devices*: Returns all the devices registered with MusicLedStudio

- *effects*: Returns all the valid schemas for an MusicLedStudio effect

- *integrations*: Returns all the integrations registered with MusicLedStudio

/api/devices
=========================

Query and manage devices connected to MusicLedStudio
...
Adds a new device to MusicLedStudio based on the provided JSON configuration
...
Returns all the effects currently created in MusicLedStudio as JSON
...
Query and manage virtuals connected to MusicLedStudio
...
Adds a new virtual to MusicLedStudio based on the provided JSON configuration
...
In addition to the REST APIs MusicLedStudio has a WebSocket API for streaming realtime data. The primary use for this is for things like effect visualizations in the frontend.
