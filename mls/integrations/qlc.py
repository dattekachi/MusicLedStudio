...
from mls.utils import async_fire_and_forget, resolve_destination
...
class QLC(Integration):
    ...
    def create_event(self, event_type, event_filter, active, qlc_payload):
        ...
        # Finally, subscribe to the mls event if the listener is now active
        if active:
    ...
    def _remove_listener(self, event_type, event_filter):
        """Internal function to remove mls events listener if it exists"""
        ...
        def make_callback(qlc_payload):
            def callback(_):
                ...
                async_fire_and_forget(
                    self._send_payload(qlc_payload), loop=self._mls.loop
                )

            return callback

        callback = make_callback(qlc_payload)
        listener = self._mls.events.add_listener(
    ...
    async def disconnect(self):
        ...
        if self._client is not None:
            ...
            async_fire_and_forget(
                self._client.disconnect(), loop=self._mls.loop
            )
