...
from mls.events import SceneActivatedEvent
...
class MQTT(Integration):
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        client.publish(
            ...
            str(self._mls.config["scenes"]),
        )
        client.publish(
            f"{self._config['topic']}/DEVICES",
            str(self._mls.config["virtuals"]),
        )
    ...
    def on_message(self, client, userdata, msg):
        ...
        if msg.topic == f"{self._config['topic']}/SCENE":
            ...
            _LOGGER.warning(str(self._mls.config["scenes"].keys()))
            if scene_id not in self._mls.config["scenes"].keys():
                response = {
                    "status": "failed",
            ...
            scene = self._mls.config["scenes"][scene_id]
            ...
            self._mls.events.fire_event(SceneActivatedEvent(scene["name"]))
            # SET SCENE END
