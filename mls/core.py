...
from mls.devices import Devices
from mls.effects import Effects
from mls.effects.math import interpolate_pixels
from mls.events import (
    Event,
    Events,
    MusicLedStudioShutdownEvent,
    VisualisationUpdateEvent,
)
from mls.http_manager import HttpServer
from mls.integrations import Integrations
from mls.mdns_manager import ZeroConfRunner
from mls.presets import mls_presets
from mls.scenes import Scenes
...
from mls.virtuals import Virtuals
...
class MusicLedStudioCore:

    EXIT_CODES = {
        1: "MusicLedStudio encountered an error - Shutting down.",
    ...
    def __init__(
        ...
        if clear_config:
            _LOGGER.warning("Clearing MusicLedStudio config.")
        ...
        self.config["mls_presets"] = mls_presets
        ...
        self.http = HttpServer(
            mls=self, host=self.host, port=self.port, port_s=self.port_s
        )
    ...
    def open_ui(self):
        ...
        try:
            ...
            _LOGGER.warning(
                f"Failed to open default web browser. To access MusicLedStudio's web ui, open {url} in your browser."
            )
    ...
    def setup_icon_menu(self):
        ...
        self.icon.menu = pystray.Menu(
            # pystray.MenuItem(
            #     f"MusicLedStudio - {PROJECT_VERSION}", None, enabled=False
            # ),
            pystray.MenuItem("Mở Led Music Studio", self.open_ui, default=True),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("Theo màu màn hình", self.run_ambilight_app),
            pystray.MenuItem("Khởi động lại", self.restart_app),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("Thoát", self.stop),
        )
    ...
    def setup_logqueue(self):
        def log_filter(record):
            return (record.name != "mls.api.log") and (record.levelno >= 20)
    ...
    def check_and_notify_updates(self, show_check_notification=None):
        """
        Checks for updates of MusicLedStudio and notifies the user if a new version is available.
        ...
        if show_check_notification:
            if self.icon and self.icon.HAS_NOTIFICATION:
                self.icon.notify("Checking for updates...", "MusicLedStudio")
        ...
        if UpdateChecker.get_release_information():
            if UpdateChecker.update_available():
                ...
                _LOGGER.warning(
                    f"New version of MusicLedStudio available: {latest_version} - {release_url}"
                )

                if self.icon and self.icon.HAS_NOTIFICATION:
                    self.icon.notify(
                        f"New version of MusicLedStudio available: {latest_version}",
                        "MusicLedStudio",
                    )
                    try:
                ...
                _LOGGER.info("MusicLedStudio is up to date.")
        else:
            _LOGGER.warning("Unable to get update information.")
            if show_check_notification:
                if self.icon and self.icon.HAS_NOTIFICATION:
                    self.icon.notify(
                        "Unable to get update information", "MusicLedStudio"
                    )
    ...
    def start(self, open_ui=False):
        ...
        try:
            ...
            # MusicLedStudio will cleanly shutdown.
    ...
    async def async_start(self, open_ui=False):
        _LOGGER.info(f"Starting MusicLedStudio, listening on {self.host}:{self.port}")
        ...
        if (
            ...
            # self.icon.notify(
            #     "Started in background.\nUse the tray icon to open.", "MusicLedStudio"
            # )
            self.open_ui()
        ...
        self.zeroconf = ZeroConfRunner(mls=self)
