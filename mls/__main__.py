#!/usr/bin/env python
"""
Entry point for MusicLedStudio.
...
from mls.sentry_config import setup_sentry
...
import mls.config as config_helpers
from mls.consts import PROJECT_VERSION
from mls.core import MusicLedStudioCore
...
if sys.platform == "win32":
    for process in psutil.process_iter():
        try:
            if process.name() == "ambilightapp.exe":
                process.terminate()
...
def parse_args():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"MusicLedStudio {PROJECT_VERSION}",
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    parser.add_argument(
        "--host",
        dest="host",
        help="The address to host MusicLedStudio web interface",
        default=None,
        type=str,
    )
    ...
    group.add_argument(
        ...
        dest="tray",
        action="store_true",
        help="Force MusicLedStudio system tray icon",
    )
    ...
    group.add_argument(
        ...
        dest="no_tray",
        action="store_true",
        help="Force no MusicLedStudio system tray icon",
    )
    ...
    parser.add_argument(
        ...
        dest="sentry_test",
        action="store_true",
        help="This crashes MusicLedStudio to test the sentry crash logger",
    )
    parser.add_argument(
        "--ci-smoke-test",
        dest="ci_smoke_test",
        action="store_true",
        help="Launch MusicLedStudio and then exit after 5 seconds to sanity check the install",
    )
    ...
    parser.add_argument(
        ...
        dest="clear_config",
        action="store_true",
        help="Launch MusicLedStudio, backup the config, clear the config, and continue with a clean startup",
    )
    ...
    parser.add_argument(
        ...
        dest="clear_effects",
        action="store_true",
        help="Launch MusicLedStudio, load the config, clear all active effects on all virtuals. Effect configurations are persisted, just turned off",
    )
...
def main():
    ...
    if args.sentry_test:
        """This will crash MusicLedStudio and submit a Sentry error if Sentry is configured"""
        _LOGGER.warning("Steering MusicLedStudio into a brick wall")
        div_by_zero = 1 / 0
    ...
    if (args.tray or currently_frozen()) and not args.no_tray:
        ...
        icon = pystray.Icon(
            "MusicLedStudio", icon=Image.open(icon_location), title="Music Led Studio"
        )
    else:
        icon = None
...
def entry_point(icon=None):
    ...
    exit_code = 4
    while exit_code == 4:
        _LOGGER.info("MusicLedStudio Core is initializing")

        mls = MusicLedStudioCore(
        ...
        exit_code = mls.start(open_ui=args.open_ui)
