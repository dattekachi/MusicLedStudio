...
class HueDevice(NetworkedDevice):
    ...
    def _hue_register(self):
        if (self._config.get("username") is None) and (
            ...
            request_data = {
                "devicetype": f"MusicLedStudio#{self._config['group_name']}",
            ...
            if "error" in response[0]:
                # Credentials are no longer valid - need Bridge Link Button to be pressed and MusicLedStudio to be restarted.
                ...
                raise Exception(
                    "You need to press the Bridge Link Button and restart MusicLedStudio."
                )
