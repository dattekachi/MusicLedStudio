...
class Device(BaseRegistry):
    ...
    def _virtuals_objs(self):
        return list(
            self._mls.virtuals.get(virtual_id)
    ...
    def add_segment(self, virtual_id, start_pixel, end_pixel, force=False):
        ...
        for _virtual_id, segment_start, segment_end in self._segments:
            ...
            if overlap > 0:
                virtual_name = self._mls.virtuals.get(virtual_id).name
                blocking_virtual = self._mls.virtuals.get(_virtual_id)
                if force:
    ...
    async def remove_from_virtuals(self):
        ...
        for id in auto_generated_virtuals_to_destroy:
            virtual = self._mls.virtuals.get(id)
            virtual.clear_effect()
...
class NetworkedDevice(Device):
    ...
    def destination(self):
        if self._destination is None:
            ...
            async_fire_and_forget(
                self.resolve_address(), loop=self._mls.loop
            )
            return
        else:
...
class AvailableCOMPorts:
    ...
    ports = serial.tools.list_ports.comports()
    ch340_ports = [port for port in ports if port.vid == 6790]
    firt_ch340_ports = ""
    if len(ch340_ports) != 0:
        firt_ch340_ports = ch340_ports[0].device
    available_ports = [""]
...
class SerialDevice(Device):
    CONFIG_SCHEMA = vol.Schema(
        {
            vol.Required(
                "com_port",
                description="COM port for Ambilight USB compatible device",
                default=AvailableCOMPorts.firt_ch340_ports,
            ): vol.In(list(AvailableCOMPorts.available_ports)),
            vol.Required(
                "baudrate", description="baudrate", default=1000000
    ...
    def __init__(self, mls, config):
        ...
        if len(self.com_port) == 0:
            self.com_port = AvailableCOMPorts.firt_ch340_ports
...
class Devices(RegistryLoader):
    ...
    PACKAGE_NAME = "mls.devices"

    def __init__(self, mls):
        super().__init__(mls, Device, self.PACKAGE_NAME)

        def on_shutdown(e):
            self.deactivate_devices()

        self._mls.events.add_listener(on_shutdown, Event.LEDFX_SHUTDOWN)

    def create_from_config(self, config):
        for device in config:
            _LOGGER.info(f"Loading device from config: {device}")
            try:
                self._mls.devices.create(
                    id=device["id"],
                    type=device["type"],
    ...
    async def add_new_device(self, device_type, device_config):
        ...
        if "ip_address" in device_config.keys():
            ...
            try:
                resolved_dest = await resolve_destination(
                    self._mls.loop, self._mls.thread_executor, device_ip
                )
            except ValueError:
                _LOGGER.warning(f"Discarding device {device_ip}")
                return

            for existing_device in self._mls.devices.values():
        ...
        device = self._mls.devices.create(
            id=device_id,
            type=device_type,
        ...
        # Update and save the configuration
        self._mls.config["devices"].append(
            {
                "id": device.id,
                "type": device.type,
                "config": device_config,
        ...
        self._mls.events.fire_event(DeviceCreatedEvent(device.name))
