...
from mls.devices import NetworkedDevice, packets
...
class OpenRGB(NetworkedDevice):
    ...
    def __init__(self, mls, config):
        ...
        Args:
            mls (MusicLedStudio): The main MusicLedStudio instance.
            config (dict): The configuration for the OpenRGB device.

        Attributes:
            _mls (MusicLedStudio): The main MusicLedStudio instance.
    ...
    def activate(self):
        ...
        if not device_supports_direct:
            _LOGGER.warning(
                f"{self.openrgb_device_id} doesn't support direct mode - not supported by MusicLedStudio."
            )
            self.set_offline()
            return
        else:
