...
from mls.devices import NetworkedDevice
from mls.devices.ddp import DDPDevice
from mls.devices.e131 import E131Device
from mls.devices.udp import UDPRealtimeDevice
from mls.utils import WLED, wled_support_DDP
...
class WLEDDevice(NetworkedDevice):
    ...
    CONFIG_SCHEMA = vol.Schema(
        {
            ...
            vol.Optional(
                "timeout",
                description="Time between MusicLedStudio effect off and WLED effect activate",
                default=1,
            ): vol.All(int, vol.Range(0, 255)),
            vol.Optional(
                "create_segments",
                description="Import WLED segments into MusicLedStudio",
                default=False,
            ): bool,
            vol.Optional(
                "icon_name",
    ...
    def __init__(self, mls, config):
        ...
        # see https://github.com/MusicLedStudio/LedFx/pull/237
        self.device_configs = {
            "UDP": {
                "name": None,
                "ip_address": None,
    ...
    def setup_subdevice(self):
        ...
        self.subdevice = device(self._mls, config)
    ...
    async def add_postamble(self):
        ...
        if (
            ...
            or self._mls.config["create_segments"]
        ):
