...
class QLCEndpoint(RestEndpoint):
    ...
    async def get(self, integration_id) -> web.Response:
        ...
        effect_names = []
        for effect_type, effect in self._mls.effects.classes().items():
            effect_names.append(effect.NAME)

        scene_ids = []
        for scene in self._mls.config["scenes"]:
            scene_ids.append(self._mls.config["scenes"][scene]["name"])
    ...
    async def post(self, integration_id, request) -> web.Response:
        ...
        # Create a link between mls event and sending the payload
