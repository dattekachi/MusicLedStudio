...
from mls.utils import empty_queue
...
class WebsocketEndpoint(RestEndpoint):
    ...
    async def get(self, request) -> web.Response:
        try:
            return await WebsocketConnection(self._mls).handle(request)
...
class WebsocketConnection:
    ...
    async def handle(self, request):
        ...
        self._receiver_task = asyncio.current_task(loop=self._mls.loop)
        self._sender_task = self._mls.loop.create_task(self._sender())

        def shutdown_handler(e):
            self.close()

        remove_listeners = self._mls.events.add_listener(
    ...
    def subscribe_event_handler(self, message):
        ...
        self._listeners[message["id"]] = self._mls.events.add_listener(
            notify_websocket,
