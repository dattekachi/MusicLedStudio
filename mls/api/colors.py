...
from mls.color import validate_color
...
class ColorEndpoint(RestEndpoint):
    ...
    async def get(self) -> web.Response:
        """
        Get MusicLedStudio colors and gradients
        ...
        response = {
            "colors": dict(
                zip(("builtin", "user"), self._mls.colors.get_all())
            ),
            "gradients": dict(
                zip(("builtin", "user"), self._mls.gradients.get_all())
            ),
        }
    ...
    async def delete(self, request: web.Request) -> web.Response:
        ...
        for key in data:
            del self._mls.colors[key]
            del self._mls.gradients[key]
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        for key, val in data.items():
            try:
                ...
                is_color = False
            if is_color:
                self._mls.colors[key] = val
            else:
                self._mls.gradients[key] = val
