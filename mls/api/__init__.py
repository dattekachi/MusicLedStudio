...
from mls.utils import BaseRegistry, RegistryLoader
...
class RestEndpoint(BaseRegistry):
    ...
    async def handler(self, request: web.Request):
        ...
        _LOGGER.debug(
            f"MusicLedStudio API Request {short_uuid}: {request.method} {request.path}"
        )
        body = None
        if request.has_body:
            try:
                ...
                _LOGGER.debug(
                    f"MusicLedStudio API Request {short_uuid} payload: {body}"
                )
...
class RestApi(RegistryLoader):
    PACKAGE_NAME = "mls.api"

    def __init__(self, mls):
        super().__init__(mls, RestEndpoint, self.PACKAGE_NAME)
        self._mls = mls
    ...
    def register_routes(self, app):
        ...
        for endpoint_type in self.types():
            endpoint = self.create(type=endpoint_type, mls=self._mls)
            resource = cors.add(
