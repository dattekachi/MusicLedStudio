...
from mls.api.utils import PERMITTED_KEYS, convertToJsonSchema
from mls.config import CORE_CONFIG_SCHEMA, WLED_CONFIG_SCHEMA
from mls.effects.audio import AudioAnalysisSource, AudioInputSource
from mls.effects.melbank import Melbank, Melbanks
        ...
        for schema in schemas:
            if schema == "devices":
                ...
                for (
                    device_type,
                    device,
                ) in self._mls.devices.classes().items():
                ...
                for (
                    effect_type,
                    effect,
                ) in self._mls.effects.classes().items():
                ...
                for (
                    integration_type,
                    integration,
                ) in self._mls.integrations.classes().items():
