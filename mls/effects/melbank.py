...
class Melbanks:
    ...
    def __init__(self, mls, audio, config):
        self._mls = mls
        self._audio = audio
        self.update_config(config)
        self.dev_enabled = self._mls.dev_enabled()

    def update_config(self, config):
        self.melbanks_config = self.CONFIG_SCHEMA(config)
        self.melbank_collection = self._mls.config.get(
            "melbank_collection", []
        )
        # set up the melbanks
        ...
        self._mls.config["melbank_collection"] = (
            self.cleaned_melbank_collection
        )
        self._mls.config["melbanks"] = self.melbanks_config
