...
CORE_CONFIG_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional("mls_presets", default={}): dict,
...
def get_profile_dump_location(config_dir: str) -> str:
    ...
    return os.path.join(config_dir, f"MusicLedStudio_{date_time}.profile")
...
def get_log_file_location(config_dir: str) -> str:
    ...
    """
    log_file_path = os.path.abspath(os.path.join(config_dir, "mls.log"))
...
def load_config(config_dir: str) -> dict:
    ...
    try:
        with open(config_file, encoding="utf-8") as file:
            ...
            try:
                _LOGGER.info(
                    f"MusicLedStudio Configuration Version: {config_json['configuration_version']}"
                )
                ...
                _LOGGER.warning(
                    f"MusicLedStudio config version: {CONFIGURATION_VERSION}, your config version: {config_json.get('configuration_version', 'UNDEFINED (old!)')}"
                )
                try:
...
def migrate_config(old_config):
    ...
    effects = Effects(DummyMls()).classes()
