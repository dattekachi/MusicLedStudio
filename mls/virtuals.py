...
from mls.effects.math import interpolate_pixels, make_pattern
from mls.effects.melbank import (
    MAX_FREQ,
    MIN_FREQ,
    MIN_FREQ_DIFFERENCE,
    FrequencyRange,
...
from mls.transitions import Transitions
from mls.utils import fps_to_sleep_interval
...
class Virtual:
    ...
    def _valid_id(self, id):
        device = self._mls.devices.get(id)
    ...
    def set_preset(self, preset_info):
        ...
        try:
            effect_config = self._mls.config[category][effect_id][preset_id][
                "config"
            ]
        except KeyError:
    ...
    def clear_effect(self):
        with self.lock:
            self._mls.events.fire_event(EffectClearedEvent())
            self.clear_transition_effect()
            ...
            self.clear_handle = self._mls.loop.call_later(
        ...
        while True:
            ...
            with self.lock:
                if (
                    ...
                    # self.assembled_frame = await self._mls.loop.run_in_executor(
                    #     self._mls.thread_executor, self.assemble_frame
                    # )
                    ...
                    if self.assembled_frame is not None and not self._paused:
                        if not self._config["preview_only"]:
                            # self._mls.thread_executor.submit(self.flush)
                            # await self._mls.loop.run_in_executor(
                            #     self._mls.thread_executor, self.flush
                            # )
                            self.flush()
    ...
    def assemble_frame(self):
        ...
        if frame is not None:
            ...
            np.multiply(frame, self._mls.config["global_brightness"], frame)
        return frame
    ...
    def activate(self):
        ...
        self._thread.start()
        self._mls.events.fire_event(VirtualPauseEvent(self.id))
        # self._task = self._mls.loop.create_task(self.thread_function())
    ...
    def _devices(self):
        ...
        """
        return list(
            self._mls.devices.get(device_id)
    ...
    def config(self, new_config):
        ...
        if hasattr(self, "_config"):
            ...
            if (
                ...
                if self._mls.config["global_transitions"]:
                    for virtual_id in self._mls.virtuals:
...
class Virtuals:
    ...
    PACKAGE_NAME = "mls.virtuals"
    _paused = False

    def __init__(self, mls):
        # super().__init__(mls, Virtual, self.PACKAGE_NAME)

        def cleanup_effects(e):
            self.clear_all_effects()

        self._mls = mls
        self._mls.events.add_listener(cleanup_effects, Event.LEDFX_SHUTDOWN)
        self._virtuals = {}

    def create_from_config(self, config):
        for virtual in config:
            _LOGGER.debug(f"Loading virtual from config: {virtual}")
            self._mls.virtuals.create(
                id=virtual["id"],
            ...
            if "segments" in virtual:
                try:
                    self._mls.virtuals.get(virtual["id"]).update_segments(
                        virtual["segments"]
                    )
            ...
            if "effect" in virtual:
                try:
                    ...
                    self._mls.virtuals.get(virtual["id"]).set_effect(effect)
                except vol.MultipleInvalid:
                    _LOGGER.warning(
    ...
    def pause_all(self):
        ...
        self._mls.events.fire_event(GlobalPauseEvent())
