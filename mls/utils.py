...
from mls.consts import LEDFX_ASSETS_PATH, PROJECT_VERSION
...
try:
    ...
    from mls.config import get_default_config_directory
...
class WLED:
    ...
    def set_inactivity_timeout(self, timeout=2.5):
        """
        Updates internal sync settings to set timeout for wled effect virtual after mls streaming finishes
...
async def resolve_destination(
    ...
    Args:
        loop: mls event loop (mls.loop)
        executor: mls executor (mls.thread_executor)
...
def read_mls_dotenv():
    """
    Loads the environment variables from the mls.env file.

    In normal development, this does not exist and thus does nothing.

    During the CI build and release process, the mls.env file is created and populated with:

    IS_RELEASE = [true|false]
    GITHUB_SHA = [commit hash]

    It is then packaged in the mls folder for distribution.
    ...
    if currently_frozen():
        ...
        path_to_load = os.path.join(extDataDir, "mls.env")
    else:
        parent_dir = os.path.dirname(extDataDir)
        path_to_load = os.path.join(parent_dir, "mls.env")
...
def get_icon_path(icon_filename) -> str:
    ...
    Assumes that the file is within mls_assets folder
...
class UserDefaultCollection(MutableMapping):
    ...
    User defined values are saved automatically in MusicLedStudio config.
    ...
    Parser is a callable that translates config values into a valid form for mls to use
    """
    ...
    def __init__(
        ...
        user: mls config key for user values
        """
        self._mls = mls
        self._collection_name = collection_name
        self._default_vals = defaults
        self._user_vals = self._mls.config[user]
    ...
    def __delitem__(self, key):
        if key in self._default_vals:
            _LOGGER.error(
                f"Cannot delete MusicLedStudio {self._collection_name}: {key}"
            )
            return
    ...
    def __setitem__(self, key, value):
        if key in self._default_vals:
            _LOGGER.error(
                f"Cannot overwrite MusicLedStudio {self._collection_name}: {key}"
            )
            return
...
class RegistryLoader:
    ...
    def __init__(self, mls, cls, package):
        ...
        # Check mls is not running as a single exe built using pyinstaller
        # (sys frozen flag).
        if mls.dev_enabled() and not currently_frozen():
...
def generate_default_config(mls_effects, effect_id):
    return mls_effects.get_class(effect_id).get_combined_default_schema()
...
def generate_defaults(mls_presets, mls_effects, effect_id):
    ...
    Args:
        mls_presets (dict): The current presets.
        mls_effects (dict): The current effects.
        effect_id (str): The ID of the effect.

    Returns:
        dict: The default presets for the effect.
    """
    if effect_id in mls_presets.keys():
        presets = mls_presets[effect_id]
    else:
        presets = {}

    default = {
        "reset": {
            "config": generate_default_config(mls_effects, effect_id),
            "name": "reset",
        }
    }
...
class PerformanceAnalysis:
    ...
    def compare_functions(
        ...
        To use, import the PerformanceAnalysis using "from mls.utils import PerformanceAnalysis"
